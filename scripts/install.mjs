...
// Return a repo-relative path for `absPath` against `repoDir`, falling back
// to the input when relative() returns "" (path === repoDir itself). The
// previous `p.startsWith(repoDir) ? p.slice(repoDir.length + 1) : p` shortcut
// broke when `repoDir === "."`: `".".startsWith(".")` matched any leading-dot
// path like `.github/foo`, so `.slice(2)` stripped two chars and we shipped
// `ithub/foo` to `git add`. relative() handles the dot-cwd case correctly.
export function relativizePath(repoDir, absPath) {
...
`;
  ...
  try {
    run("gh", ["pr", "create", "--head", branch, "--title", "chore(pipeline-core): install reusable pipeline", "--body", body], { cwd: repoDir });
  } catch (err) {
...
async function main() {
  ...
  for (const p of allFiles) process.stdout.write(`  ${relativizePath(repoDir, p)}\n`);
