...
> `deploy/pipeline-<slug>/` is reserved for future opt-in self-hosting (a repo that builds its own runner pool in place). The pipeline runs keyless on GitHub-hosted runners by default, so no `deploy/` source exists today.
